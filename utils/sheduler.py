...
class CyclicCosAnnealingLR(_LRScheduler):
	...
	def __init__(self, optimizer, milestones, eta_min=0, scale=1, last_epoch=-1):
	...
	def get_lr(self):
		...
		if self.last_epoch > 0 & self.last_epoch in self.milestones:
			self.scale = self.scale / 10

		return [self.eta_min + (base_lr*self.scale - self.eta_min) *
