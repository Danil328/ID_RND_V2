...
from Dataset.id_rnd_dataset import CustomDataset
...
def train(model_name, optim='adam'):
    ...
    if model_name == 'EF':
        ...
        #     f"../output/models_weights/pretrained/{model_name}_{8}_1.5062978111598622_0.9967353313006619.pth"))
        ...
        #     torch.load(f"../output/models_weights/pretrained/{model_name}_{8}_1.6058124488733547_1.0.pth"))
    ...
    for epoch in trange(config['number_epochs']):
        if epoch == 3:
            train_dataset = CustomDataset(fold=fold, mode=config['mode'],
                                          double_loss_mode=True,
                                          output_shape=config['image_resolution'],
                                          aug=[0.0, 0.5, 0.0])
...
def evaluate(model, val_loader, epoch, model_name):
    ...
    torch.save(model.state_dict(),
               f"../output/models_weights/{fold}/{model_name}_{epoch}_{score:.6f}_{user_score:.6f}.pth")
...
if __name__ == '__main__':
    ...
    model_names = ['EFGAP', 'EF', 'EFGAP', 'EF', 'EFGAP']
