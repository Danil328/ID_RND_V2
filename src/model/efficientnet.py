...
class TwoHeadModel(nn.Module):
    def __init__(self, base_model, dropout=0.25, features=1000, head1_classes=4, head2_classes=1):
        super(TwoHeadModel, self).__init__()
        self.base_model = base_model

        self.block1 = nn.Sequential(
            nn.Dropout(p=dropout),
            nn.Linear(in_features=features, out_features=head1_classes),
        )
        self.block2 = nn.Sequential(
            nn.Dropout(p=dropout),
            nn.Linear(in_features=features, out_features=head2_classes),
            nn.Sigmoid()
        )
...
class EfficientNetMod(nn.Module):
    """
    ...
    def from_name(cls, model_name, override_params=None):
        ...
        return EfficientNetMod(blocks_args, global_params)

    @classmethod
    def from_pretrained(cls, model_name):
        model = EfficientNetMod.from_name(model_name)
